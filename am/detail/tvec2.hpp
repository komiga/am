...
#ifndef AM_DETAIL_TVEC2_HPP_
#define AM_DETAIL_TVEC2_HPP_
...
/** @name Constructors */ /// @{
	...
	*/
	inline explicit tvec2(ctor_no_init) {}

	/**
		Construct all components to value.
		@param s Value.
	*/
	inline explicit tvec2(value_type const& s) : x(s), y(s) {}
	/**
		Construct all components to value.
	...
	*/
	template<typename U>
	inline explicit tvec2(U const& s) : x(T(s)), y(T(s)) {}
	...
	/**
		...
		@param c2 Y value.
	*/
	inline explicit tvec2(value_type const& c1, value_type const& c2) : x(c1), y(c2) {}
	/**
		Construct to values.
	...
	*/
	...
	inline explicit tvec2(U const& c1, V const& c2) : x(T(c1)), y(T(c2)) {}

	/**
		Construct to vector.
		@param v Vector to copy.
	*/
	inline tvec2(type const& v) : x(v.x), y(v.y) {}
	/**
		Construct to vector.
		@tparam U An arithmetic type.
		@param v Vector to copy.
	*/
	template<typename U>
	inline tvec2(tvec2<U> const& v) : x(T(v.x)), y(T(v.y)) {}
	...
	*/
	template<typename U>
	inline explicit tvec2(tvec3<U> const& v) : x(T(v.x)), y(T(v.y)) {}
	/** @copydoc tvec2(tvec3<U> const&) */
	template<typename U>
	inline explicit tvec2(tvec4<U> const& v) : x(T(v.x)), y(T(v.y)) {}
/// @}
...
/** @name Properties */ /// @{
	/**
		...
		@returns @c 2.
	*/
	inline AM_CONSTEXPR size_type length() const { return size_type(2); }
	/**
		Get value at index.
...
/** @name Assignment operators */ /// @{
	...
	*/
	template<typename U>
	inline type& operator=(tvec2<U> const& v) {
		this->x=T(v.x);
		this->y=T(v.y);
		return *this;
	}
...
/** @name Increment and decrement operators */ /// @{
	...
	*/
	template<typename T>
	inline tvec2<T> operator++(tvec2<T> const& x, int) {
