...
#ifndef AM_DETAIL_TMAT4X4_HPP_
#define AM_DETAIL_TMAT4X4_HPP_
...
/** @cond INTERNAL */
	...
	inline static type inverse(type_cref m) {
		...
		value_type const det=row_type::operations::dot(m.data[0], row_type{invm.data[0].x, invm.data[1].x, invm.data[2].x, invm.data[3].x});
		invm /= det;
		return invm;
	}
	...
	inline static tmat2x4<T> multiply(type_cref m, tmat2x4<T> const& n) {
		return tmat2x4<T>{
	...
	inline static tmat3x4<T> multiply(type_cref m, tmat3x4<T> const& n) {
		return tmat3x4<T>{
			m.data[0].x * n.data[0].x + m.data[1].x * n.data[0].y + m.data[2].x * n.data[0].z + m.data[3].x * n.data[0].w,
			m.data[0].y * n.data[0].x + m.data[1].y * n.data[0].y + m.data[2].y * n.data[0].z + m.data[3].y * n.data[0].w,
			m.data[0].z * n.data[0].x + m.data[1].z * n.data[0].y + m.data[2].z * n.data[0].z + m.data[3].z * n.data[0].w,
...
/** @name Constructors */ /// @{
	...
	*/
	inline tmat4x4() /** @cond INTERNAL */: data{
		col_type{T(1), T(0), T(0), T(0)},
		col_type{T(0), T(1), T(0), T(0)},
		col_type{T(0), T(0), T(1), T(0)},
		col_type{T(0), T(0), T(0), T(1)}}/** @endcond */ {}
	/**
		Construct uninitialized.
	*/
	inline explicit tmat4x4(ctor_no_init) {}

	/**
		Construct to main diagonal.
		@param s Value.
	*/
	inline explicit tmat4x4(value_type const& s) /** @cond INTERNAL */: data{
		col_type{s, T(0), T(0), T(0)},
		col_type{T(0), s, T(0), T(0)},
		col_type{T(0), T(0), s, T(0)},
		col_type{T(0), T(0), T(0), s}}/** @endcond */ {}
	/**
		Construct to main diagonal.
	...
	*/
	template<typename U>
	inline explicit tmat4x4(U const& s) /** @cond INTERNAL */: data{
		col_type{T(s), T(0), T(0), T(0)},
		col_type{T(0), T(s), T(0), T(0)},
		col_type{T(0), T(0), T(s), T(0)},
		col_type{T(0), T(0), T(0), T(s)}}/** @endcond */ {}
	...
	/**
		...
		@param w1 W value of first column.
		...
		@param w3 W value of third column.
		@param x4 X value of fourth column.
		@param y4 Y value of fourth column.
		@param z4 Z value of fourth column.
		@param w4 W value of fourth column.
	*/
	inline explicit tmat4x4(value_type const& x1, value_type const& y1, value_type const& z1, value_type const& w1,
							value_type const& x2, value_type const& y2, value_type const& z2, value_type const& w2,
							value_type const& x3, value_type const& y3, value_type const& z3, value_type const& w3,
							value_type const& x4, value_type const& y4, value_type const& z4, value_type const& w4) /** @cond INTERNAL */: data{
		col_type{x1, y1, z1, w1},
		col_type{x2, y2, z2, w2},
		col_type{x3, y3, z3, w3},
		col_type{x4, y4, z4, w4}}/** @endcond */ {}
	/**
		Construct to values.
		@tparam X1,Y1,Z1,W1, X2,Y2,Z2,W2, X3,Y3,Z3,W3, X4,Y4,Z4,W4 An arithmetic type.
		@param x1 X value of first column.
		@param y1 Y value of first column.
		@param z1 Z value of first column.
		@param w1 W value of first column.
	...
	*/
	...
	inline explicit tmat4x4(X1 const& x1, Y1 const& y1, Z1 const& z1, W1 const& w1,
							X2 const& x2, Y2 const& y2, Z2 const& z2, W2 const& w2,
							X3 const& x3, Y3 const& y3, Z3 const& z3, W3 const& w3,
							X4 const& x4, Y4 const& y4, Z4 const& z4, W4 const& w4) /** @cond INTERNAL */: data{
		col_type{T(x1), T(y1), T(z1), T(w1)},
		col_type{T(x2), T(y2), T(z2), T(w2)},
		col_type{T(x3), T(y3), T(z3), T(w3)},
		col_type{T(x4), T(y4), T(z4), T(w4)}}/** @endcond */ {}
	...
	*/
	inline explicit tmat4x4(col_type const& c1,
							col_type const& c2,
							col_type const& c3,
	...
	*/
	...
	inline explicit tmat4x4(tvec4<C1> const& c1,
							tvec4<C2> const& c2,
							tvec4<C3> const& c3,
							tvec4<C4> const& c4) /** @cond INTERNAL */: data{
		col_type{c1},
		col_type{c2},
		col_type{c3},
		col_type{c4}}/** @endcond */ {}
	...
	*/
	inline tmat4x4(type const& m) /** @cond INTERNAL */: data{
		m.data[0],
		m.data[1],
		m.data[2],
		m.data[3]}/** @endcond */ {}
	...
	*/
	template<typename U>
	inline tmat4x4(tmat4x4<U> const& m) /** @cond INTERNAL */: data{
		col_type{m.data[0]},
		col_type{m.data[1]},
		col_type{m.data[2]},
...
/** @name Assignment operators */ /// @{
	...
	*/
	template<typename U>
	inline type& operator=(tmat4x4<U> const& m) {
		data[0]=m.data[0];
		data[1]=m.data[1];
		data[2]=m.data[2];
		data[3]=m.data[3];
...
/** @name mat4x4 increment and decrement operators */ /// @{
	...
	*/
	template<typename T>
	inline tmat4x4<T> operator++(tmat4x4<T> const& m, int) {
...
/** @name mat4x4 arithmetic operators */ /// @{
	...
	*/
	template<typename T>
	inline typename tmat4x4<T>::col_type operator*(tmat4x4<T> const& m, typename tmat4x4<T>::row_type const& v) {
	...
	*/
	template<typename T>
	inline typename tmat4x4<T>::row_type operator*(typename tmat4x4<T>::col_type const& v, tmat4x4<T> const& m) {
