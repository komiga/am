/**
@file detail/tmat2x2.hpp
@brief 2x2 matrix.

@author Tim Howard
@copyright 2012 Tim Howard under the MIT license; see @ref index or the accompanying LICENSE file for full text.
*/

#ifndef AM_DETAIL_TMAT2X2_HPP_
#define AM_DETAIL_TMAT2X2_HPP_

#include "../config.hpp"
#include "./type_traits.hpp"

#include <cassert>
#include <type_traits>

namespace am {
namespace detail {

// Forward declarations
/** @cond INTERNAL */
template<typename T> struct tvec2;
template<typename T> class tmat2x2;
template<typename T> class tmat3x2;
template<typename T> class tmat4x2;

AM_DETAIL_TYPE_IS_MATRIX(tmat2x2);
AM_DETAIL_TYPE_IS_SQUARE_MATRIX(tmat2x2);
/** @endcond */

/**
	@addtogroup linear
	@{
*/
/**
	@addtogroup matrix
	@{
*/
/**
	@addtogroup mat2x2
	@{
*/

/**
	Generic 2x2 matrix.
	@tparam T A floating-point type.
*/
template<typename T>
class tmat2x2 {
public:
	/** @cond INTERNAL */
	AM_STATIC_ASSERT(true==std::is_floating_point<T>::value, "T must be a floating-point type");
	/** @endcond */

	/** Type of @c *this. */
	typedef tmat2x2<T> type;
	/** Type of transpose. */
	typedef tmat2x2<T> transpose_type;
	/** Type of components. */
	typedef T value_type;
	/** Type of rows. */
	typedef tvec2<T> row_type;
	/** Type of columns. */
	typedef tvec2<T> col_type;
	/** Size/length type. */
	typedef std::size_t size_type;

	/** Dummy enum for constructing uninitialized matrices. */
	enum ctor_no_init {no_init};

private:
	col_type cols[2];

public:
/** @cond INTERNAL */
	struct operations;
	friend struct type::operations;
	struct operations {
	typedef type const& type_cref;
	typedef value_type const& value_cref;
	typedef row_type const& row_cref;
	typedef col_type const& col_cref;

	inline static transpose_type transpose(type_cref m) {
		return transpose_type{
			m.cols[0].x, m.cols[1].x,
			m.cols[0].y, m.cols[1].y};
	}
	inline static value_type determinant(type_cref m) {
		return 
			m.cols[0].x * m.cols[1].y -
			m.cols[1].x * m.cols[0].y;
	}
	inline static type inverse(type_cref m) {
		value_type const det=operations::determinant(m);
		return type{
			 m.cols[1].y / det, -m.cols[0].y / det,
			-m.cols[1].x / det,  m.cols[0].x / det};
	}

	inline static type scalar_add(type_cref m, value_cref s) {
		return type{
			m.cols[0] + s,
			m.cols[1] + s};
	}

	inline static type add(type_cref m, type_cref n) {
		return type{
			m.cols[0] + n.cols[0],
			m.cols[1] + n.cols[1]};
	}

	inline static type scalar_subtract_rhs(type_cref m, value_cref s) {
		return type{
			m.cols[0] - s,
			m.cols[1] - s};
	}
	inline static type scalar_subtract_lhs(type_cref m, value_cref s) {
		return type{
			s - m.cols[0],
			s - m.cols[1]};
	}

	inline static type subtract(type_cref m, type_cref n) {
		return type{
			m.cols[0] - n.cols[0],
			m.cols[1] - n.cols[1]};
	}

	inline static type scalar_multiply(type_cref m, value_cref s) {
		return type{
			m.cols[0] * s,
			m.cols[1] * s};
	}

	inline static col_type row_multiply(type_cref m, row_cref v) {
		return col_type{
			m.cols[0].x * v.x + m.cols[1].x * v.y,
			m.cols[0].y * v.x + m.cols[1].y * v.y};
	}
	inline static row_type col_multiply(type_cref m, col_cref v) {
		return row_type{
			m.cols[0].x * v.x + m.cols[0].y * v.y,
			m.cols[1].x * v.x + m.cols[1].y * v.y};
	}

	inline static type multiply(type_cref m, type_cref n) {
		return type{
			m.cols[0].x * n.cols[0].x + m.cols[1].x * n.cols[0].y,
			m.cols[0].y * n.cols[0].x + m.cols[1].y * n.cols[0].y,
			m.cols[0].x * n.cols[1].x + m.cols[1].x * n.cols[1].y,
			m.cols[0].y * n.cols[1].x + m.cols[1].y * n.cols[1].y};
	}
	inline static tmat3x2<T> multiply(type_cref m, tmat3x2<T> const& n) {
		return tmat3x2<T>{
			m.cols[0].x * n.cols[0].x + m.cols[1].x * n.cols[0].y,
			m.cols[0].y * n.cols[0].x + m.cols[1].y * n.cols[0].y,
			m.cols[0].x * n.cols[1].x + m.cols[1].x * n.cols[1].y,
			m.cols[0].y * n.cols[1].x + m.cols[1].y * n.cols[1].y,
			m.cols[0].x * n.cols[2].x + m.cols[1].x * n.cols[2].y,
			m.cols[0].y * n.cols[2].x + m.cols[1].y * n.cols[2].y};
	}
	inline static tmat4x2<T> multiply(type_cref m, tmat4x2<T> const& n) {
		return tmat4x2<T>{
			m.cols[0].x * n.cols[0].x + m.cols[1].x * n.cols[0].y,
			m.cols[0].y * n.cols[0].x + m.cols[1].y * n.cols[0].y,
			m.cols[0].x * n.cols[1].x + m.cols[1].x * n.cols[1].y,
			m.cols[0].y * n.cols[1].x + m.cols[1].y * n.cols[1].y,
			m.cols[0].x * n.cols[2].x + m.cols[1].x * n.cols[2].y,
			m.cols[0].y * n.cols[2].x + m.cols[1].y * n.cols[2].y,
			m.cols[0].x * n.cols[3].x + m.cols[1].x * n.cols[3].y,
			m.cols[0].y * n.cols[3].x + m.cols[1].y * n.cols[3].y};
	}

	inline static type scalar_divide_rhs(type_cref m, value_cref s) {
		return type{
			m.cols[0] / s,
			m.cols[1] / s};
	}
	inline static type scalar_divide_lhs(type_cref m, value_cref s) {
		return type{
			s / m.cols[0],
			s / m.cols[1]};
	}

	inline static col_type row_divide(type_cref m, row_cref v) {
		return row_multiply(operations::inverse(m), v);
	}
	inline static row_type col_divide(type_cref m, col_cref v) {
		return col_multiply(operations::inverse(m), v);
	}

	inline static type divide(type_cref m, type_cref n) {
		return multiply(m, operations::inverse(n));
	}
	};
/** @endcond */ // INTERNAL

/** @name Constructors */ /// @{
	/**
		Construct to identity.
	*/
	inline tmat2x2() /** @cond INTERNAL */: cols{
		col_type{1, 0},
		col_type{0, 1}}/** @endcond */ {}
	/**
		Construct uninitialized.
	*/
	inline explicit tmat2x2(ctor_no_init) {}

	/**
		Construct to main diagonal.
		@param s Value.
	*/
	inline explicit tmat2x2(value_type const& s) /** @cond INTERNAL */: cols{
		col_type{s, 0},
		col_type{0, s}}/** @endcond */ {}
	/**
		Construct to main diagonal.
		@tparam U An arithmetic type.
		@param s Value.
	*/
	template<typename U>
	inline explicit tmat2x2(U const& s) /** @cond INTERNAL */: cols{
		col_type{s, 0},
		col_type{0, s}}/** @endcond */ {}

	/**
		Construct to values.
		@param x1 X value of first column.
		@param y1 Y value of first column.
		@param x2 X value of second column.
		@param y2 Y value of second column.
	*/
	inline explicit tmat2x2(value_type const& x1, value_type const& y1,
							value_type const& x2, value_type const& y2) /** @cond INTERNAL */: cols{
		col_type{x1, y1},
		col_type{x2, y2}}/** @endcond */ {}
	/**
		Construct to values.
		@tparam U, V, H, L An arithmetic type.
		@param x1 X value of first column.
		@param y1 Y value of first column.
		@param x2 X value of second column.
		@param y2 Y value of second column.
	*/
	template<typename U, typename V, typename H, typename L>
	inline explicit tmat2x2(U const& x1, V const& y1,
							H const& x2, L const& y2) /** @cond INTERNAL */: cols{
		col_type{T(x1), T(y1)},
		col_type{T(x2), T(y2)}}/** @endcond */ {}

	/**
		Construct to vector columns.
		@param c1 First column.
		@param c2 Second column.
	*/
	inline explicit tmat2x2(col_type const& c1,
							col_type const& c2) /** @cond INTERNAL */: cols{
		c1,
		c2}/** @endcond */ {}
	/**
		Construct to vector columns.
		@param c1 First column.
		@param c2 Second column.
	*/
	template<typename U, typename V>
	inline explicit tmat2x2(tvec2<U> const& c1,
							tvec2<V> const& c2) /** @cond INTERNAL */: cols{
		col_type{c1},
		col_type{c2}}/** @endcond */ {}

	/**
		Construct to matrix.
		@param m Matrix to copy.
	*/
	inline tmat2x2(type const& m) /** @cond INTERNAL */: cols{
		m.cols[0],
		m.cols[1]}/** @endcond */ {}
	/**
		Construct to matrix.
		@tparam U An arithmetic type.
		@param m Matrix to copy.
	*/
	template<typename U>
	inline tmat2x2(tmat2x2<U> const& m) /** @cond INTERNAL */: cols{
		col_type{m.cols[0]},
		col_type{m.cols[1]}}/** @endcond */ {}
/// @}

/** @name Properties */ /// @{
	/**
		Get number of columns.
		@returns @c 2.
	*/
	inline AM_CONSTEXPR size_type size() const { return size_type(2); }
	/**
		Get number of rows.
		@returns @c 2.
	*/
	inline AM_CONSTEXPR size_type row_size() const { return size_type(2); }
	/**
		Get column at index.
		@returns The column at @a i.
		@param i Index to retrieve.
	*/
	inline col_type& operator[](size_type const& i) {
		assert(size()>i);
		return cols[i];
	}
	/** @copydoc operator[](size_type const&) */
	inline col_type const& operator[](size_type const& i) const {
		assert(size()>i);
		return cols[i];
	}
/// @}

/** @name Comparison operators */ /// @{
	/**
		Equivalence operator.
		@returns @c true if the two matrices are equal, @c false if they are not.
		@param m Matrix to compare with.
	*/
	inline bool operator==(type const& m) const {
		return
			cols[0]==m.cols[0] &&
			cols[1]==m.cols[1];
	}
	/**
		Non-equivalence operator.
		@returns @c false if the two matrices are equal, @c true if they are not.
		@param m Matrix to compare with.
	*/
	inline bool operator!=(type const& m) const {
		return
			cols[0]!=m.cols[0] ||
			cols[1]!=m.cols[1];
	}
/// @}

/** @name Increment and decrement operators */ /// @{
	/**
		Prefix increment.
		@returns @c *this after operation.
	*/
	inline type& operator++() {
		++cols[0];
		++cols[1];
		return *this;
	}
	/**
		Prefix decrement.
		@returns @c *this after operation.
	*/
	inline type& operator--() {
		--cols[0];
		--cols[1];
		return *this;
	}
/// @}

/** @name Assignment operators */ /// @{
	/**
		Assign to matrix.
		@returns @c *this after assignment.
		@tparam U An arithmetic type.
		@param m Matrix to copy.
	*/
	template<typename U>
	inline type& operator=(tmat2x2<U> const& m) {
		cols[0]=m.cols[0];
		cols[1]=m.cols[1];
		return *this;
	}
/// @}

/** @name Arithmetic assignment operators */ /// @{
	/**
		Add value to all components.
		@returns @c *this after operation.
		@tparam U An arithmetic type.
		@param s Value to add.
	*/
	template<typename U>
	inline type& operator+=(U const& s) {
		cols[0]+=T(s);
		cols[1]+=T(s);
		return *this;
	}
	/**
		Add matrix.
		@returns @c *this after operation.
		@tparam U An arithmetic type.
		@param m Matrix to add.
	*/
	template<typename U>
	inline type& operator+=(tmat2x2<U> const& m) {
		cols[0]+=m.cols[0];
		cols[1]+=m.cols[1];
		return *this;
	}
	/**
		Subtract value from all components.
		@returns @c *this after operation.
		@tparam U An arithmetic type.
		@param s Value to subtract.
	*/
	template<typename U>
	inline type& operator-=(U const& s) {
		cols[0]-=T(s);
		cols[1]-=T(s);
		return *this;
	}
	/**
		Subtract matrix.
		@returns @c *this after operation.
		@tparam U An arithmetic type.
		@param m Matrix to subtract.
	*/
	template<typename U>
	inline type& operator-=(tmat2x2<U> const& m) {
		cols[0]-=m.cols[0];
		cols[1]-=m.cols[1];
		return *this;
	}
	/**
		Multiply by scalar.
		@returns @c *this after operation.
		@tparam U An arithmetic type.
		@param s Scalar.
	*/
	template<typename U>
	inline type& operator*=(U const& s) {
		cols[0]*=T(s);
		cols[1]*=T(s);
		return *this;
	}
	/**
		Multiply by matrix (proper product).
		@returns @c *this after operation.
		@tparam U An arithmetic type.
		@param m Matrix to multiply by.
	*/
	template<typename U>
	inline type& operator*=(tmat2x2<U> const& m) {
		return (*this = *this * m);
	}
	/**
		Divide all components by value.
		@returns @c *this after operation.
		@tparam U An arithmetic type.
		@param s Value to divide by.
	*/
	template<typename U>
	inline type& operator/=(U const& s) {
		cols[0]/=T(s);
		cols[1]/=T(s);
		return *this;
	}
	/**
		Divide by matrix (proper quotient).
		@returns @c *this after operation.
		@tparam U An arithmetic type.
		@param m Matrix to divide by.
	*/
	template<typename U>
	inline type& operator/=(tmat2x2<U> const& m) {
		return (*this = *this / m);
	}
/// @}
};

/** @name mat2x2 increment and decrement operators */ /// @{
	/**
		Matrix postfix increment.
		@returns New matrix with @c m+1.
		@tparam T An arithmetic type.
		@param m Matrix.
	*/
	template<typename T>
	inline tmat2x2<T> operator++(tmat2x2<T> const& m, int) {
		return tmat2x2<T>{
			m[0]++,
			m[1]++};
	}
	/**
		Matrix postfix decrement.
		@returns New matrix with @c m-1.
		@tparam T An arithmetic type.
		@param m Matrix.
	*/
	template<typename T>
	inline tmat2x2<T> operator--(tmat2x2<T> const& m, int) {
		return tmat2x2<T>{
			m[0]--,
			m[1]--};
	}
/// @}

/** @name mat2x2 unary operators */ /// @{
	/**
		Matrix unary plus.
		@returns New matrix with exact value of @a m.
		@tparam T An arithmetic type.
		@param m Matrix.
	*/
	template<typename T>
	inline tmat2x2<T> operator+(tmat2x2<T> const& m) {
		return tmat2x2<T>{
			m[0],
			m[1]};
	}
	/**
		Matrix unary minus.
		@returns New matrix with @c -m.
		@tparam T An arithmetic type.
		@param m Matrix.
	*/
	template<typename T>
	inline tmat2x2<T> operator-(tmat2x2<T> const& m) {
		return tmat2x2<T>{
			-m[0],
			-m[1]};
	}
/// @}

/** @name mat2x2 arithmetic operators */ /// @{
	/**
		Matrix right-hand value addition (component-wise).
		@returns New matrix with @a m plus @a s.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param s Value (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator+(tmat2x2<T> const& m, T const& s) {
		return tmat2x2<T>::operations::scalar_add(m, s);
	}
	/**
		Matrix left-hand value addition (component-wise).
		@returns New matrix with @a s plus @a m.
		@tparam T An arithmetic type.
		@param s Value (left-hand).
		@param m Matrix (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator+(T const& s, tmat2x2<T> const& m) {
		return tmat2x2<T>::operations::scalar_add(m, s);
	}
	/**
		Matrix addition.
		@returns New matrix with @a m plus @a n.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param n Matrix (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator+(tmat2x2<T> const& m, tmat2x2<T> const& n) {
		return tmat2x2<T>::operations::add(m, n);
	}
	/**
		Matrix right-hand value subtraction (component-wise).
		@returns New matrix with @a m minus @a s.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param s Value (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator-(tmat2x2<T> const& m, T const& s) {
		return tmat2x2<T>::operations::scalar_subtract_rhs(m, s);
	}
	/**
		Matrix left-hand value subtraction (component-wise).
		@returns New matrix with @a s minus @a m.
		@tparam T An arithmetic type.
		@param s Value (left-hand).
		@param m Matrix (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator-(T const& s, tmat2x2<T> const& m) {
		return tmat2x2<T>::operations::scalar_subtract_lhs(m, s);
	}
	/**
		Matrix subtraction.
		@returns New matrix with @a m minus @a n.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param n Matrix (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator-(tmat2x2<T> const& m, tmat2x2<T> const& n) {
		return tmat2x2<T>::operations::subtract(m, n);
	}
	/**
		Matrix right-hand scalar multiplication (component-wise).
		@returns New matrix with @a m times @a s.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param s Scalar (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator*(tmat2x2<T> const& m, T const& s) {
		return tmat2x2<T>::operations::scalar_multiply(m, s);
	}
	/**
		Matrix left-hand scalar multiplication (component-wise).
		@returns New matrix with @a s times @a m.
		@tparam T An arithmetic type.
		@param s Scalar (left-hand).
		@param m Matrix (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator*(T const& s, tmat2x2<T> const& m) {
		return tmat2x2<T>::operations::scalar_multiply(m, s);
	}
	/**
		Matrix right-hand (row) vector multiplication (proper product).
		@returns New column vector with @a m times @a v.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param v Vector (right-hand).
	*/
	template<typename T>
	inline typename tmat2x2<T>::col_type operator*(tmat2x2<T> const& m, typename tmat2x2<T>::row_type const& v) {
		return tmat2x2<T>::operations::row_multiply(m, v);
	}
	/**
		Matrix left-hand (column) vector multiplication (proper product).
		@returns New row vector with @a v times @a m.
		@tparam T An arithmetic type.
		@param v Vector (left-hand).
		@param m Matrix (right-hand).
	*/
	template<typename T>
	inline typename tmat2x2<T>::row_type operator*(typename tmat2x2<T>::col_type const& v, tmat2x2<T> const& m) {
		return tmat2x2<T>::operations::col_multiply(m, v);
	}
	/**
		Matrix multiplication (proper product).
		@returns New matrix with @a m times @a n.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param n Matrix (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator*(tmat2x2<T> const& m, tmat2x2<T> const& n) {
		return tmat2x2<T>::operations::multiply(m, n);
	}
	/**
		Matrix multiplication (proper product).
		@returns New matrix with @a m times @a n.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param n Matrix (right-hand).
	*/
	template<typename T>
	inline tmat3x2<T> operator*(tmat2x2<T> const& m, tmat3x2<T> const& n) {
		return tmat2x2<T>::operations::multiply(m, n);
	}
	/**
		Matrix multiplication (proper product).
		@returns New matrix with @a m times @a n.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param n Matrix (right-hand).
	*/
	template<typename T>
	inline tmat4x2<T> operator*(tmat2x2<T> const& m, tmat4x2<T> const& n) {
		return tmat2x2<T>::operations::multiply(m, n);
	}
	/**
		Matrix right-hand value division (component-wise).
		@returns New matrix with @a m divided by @a s.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param s Value (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator/(tmat2x2<T> const& m, T const& s) {
		return tmat2x2<T>::operations::scalar_divide_rhs(m, s);
	}
	/**
		Matrix left-hand value division (component-wise).
		@returns New matrix with @a s divided by @a m.
		@tparam T An arithmetic type.
		@param s Value (left-hand).
		@param m Matrix (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator/(T const& s, tmat2x2<T> const& m) {
		return tmat2x2<T>::operations::scalar_divide_lhs(m, s);
	}
	/**
		Matrix right-hand vector division (proper quotient).
		@returns New column vector with @a m divided by @a v.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param v Vector (right-hand).
	*/
	template<typename T>
	inline typename tmat2x2<T>::col_type operator/(tmat2x2<T> const& m, typename tmat2x2<T>::row_type const& v) {
		return tmat2x2<T>::operations::row_divide(m, v);
	}
	/**
		Matrix left-hand vector division (proper quotient).
		@returns New row vector with @a v divided by @a m.
		@tparam T An arithmetic type.
		@param v Vector (left-hand).
		@param m Matrix (right-hand).
	*/
	template<typename T>
	inline typename tmat2x2<T>::row_type operator/(typename tmat2x2<T>::col_type const& v, tmat2x2<T> const& m) {
		return tmat2x2<T>::operations::col_divide(m, v);
	}
	/**
		Matrix division (proper quotient).
		@returns New matrix with @a m divided by @a n.
		@tparam T An arithmetic type.
		@param m Matrix (left-hand).
		@param n Matrix (right-hand).
	*/
	template<typename T>
	inline tmat2x2<T> operator/(tmat2x2<T> const& m, tmat2x2<T> const& n) {
		return tmat2x2<T>::operations::divide(m, n);
	}
/// @}

/** @} */ // end of doc-group mat2x2
/** @} */ // end of doc-group matrix
/** @} */ // end of doc-group linear

} // namespace detail
} // namespace am

#endif // AM_DETAIL_TMAT2X2_HPP_
