...
#ifndef AM_VECTOR_OPERATIONS_HPP_
#define AM_VECTOR_OPERATIONS_HPP_
...
#define AM_VEC_OP_REQUIRE_FLOATING_POINT(conType)\
	AM_STATIC_ASSERT(detail::is_vector<conType>::value,\
		"conType must be a vector");\
	AM_STATIC_ASSERT(detail::is_construct_floating_point<conType>::value,\
...
*/
template<typename T>
inline detail::tvec3<T> cross(detail::tvec3<T> const& v, detail::tvec3<T> const& r) {
	AM_VEC_OP_REQUIRE_FLOATING_POINT(detail::tvec3<T>);
	return detail::tvec3<T>::operations::cross(v, r);
}
...
#endif // AM_VECTOR_OPERATIONS_HPP_
